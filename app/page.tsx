...
function riskScoreFromMchat(template: any, respostas: Record<string,string>) {
  ...
  for (const item of template.itens) {
    ...
    if (v === "") continue;
    const escolha = v === "0" ? "Sim" : "Não"; // 0->Sim, 1->Não
...
export default function Page() {
  ...
  const [admin, setAdmin] = React.useState({
    ...
    templateId: "mchat",
    destinatarioTipo: "responsavel", // responsavel | professor | profissional
    profissionalSubtipo: "", // pediatra | psicologo | psicopedagogo | psiquiatra
  });
  ...
  function submitResponse(code: string, data: Record<string,string>) {
    ...
    if (!a) return;
    const next = { ...store, assignments: { ...assignments, [code]: { ...a, responses: data, status: "concluido", submittedAt: todayISO() } } };
    setStore(next);
  }
        ...
        {role === "admin" && (
          <div className="space-y-6">
                ...
                <div className="grid-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select className="select" value={admin.templateId} onChange={e=>setAdmin(s=>({...s, templateId:e.target.value}))}>
                      {templates.filter(t => domainFilter==="todos" || (t.tags||[]).includes(domainFilter))
                        .map(t => (<option key={t.id} value={t.id}>{t.nome}</option>))}
                    </select>
                    <p className="text-xs text-slate-500 mt-2">Descrição: {templates.find(t=>t.id===admin.templateId)?.descricao}</p>
                  </div>
                    ...
                    <div key={a.id} className="rounded-lg border border-slate-200 p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-xs text-slate-500">{templatesById[a.templateId]?.nome}</div>
                        </div>
            ...
            <div className="card">
              ...
              <div className="card-content"><QuickTemplateForm onCreate={(t:any)=> setTemplates((prev:any) => [...prev, t])} domains={domains} /></div>
            </div>
          </div>
        )}
        ...
        {role === "guest" && (
          <div className="card">
            ...
            <div className="card-content space-y-4">
              ...
              <Responder code={inviteCode} templatesById={templatesById} assignment={assignments[inviteCode]} onSubmit={submitResponse} />
            </div>
          </div>
        )}
        ...
        <div key={item.id} className="rounded-xl border bg-white p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {template.escala.map((opt:string, idx:number) => (
              <button key={idx}
                className={(form[item.id] === String(idx) ? "btn" : "btn-outline") + " text-sm"}
